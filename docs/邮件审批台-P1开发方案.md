...
当前邮件中心（`client/src/pages/Emails.js`）只有前端壳：`emailApi.js` 中 `USE_MOCK = true`，回复待确认、发送记录、模板管理、邮箱配置四个 Tab 全是内置假数据，服务端不存在 `/api/emails/*` 任何路由。

我们已经验证过有效的外联方式：AI 先调查达人（近 30 天真实视频、播放数据、匹配理由），再据此撰写个性化首触邮件，而不是通用模板填变量。本方案把这个流程做进系统，建成"邮件审批台"：
...
5. 回复接入：IMAP 轮询收件、AI 摘要与意向分类（落实现有 mock 设计）、回复草稿进入同一审批队列。
6. 跟进自动化：发送后 48 小时未回复生成跟进草稿进审批队列；5 天未回复标记降级建议。
7. 状态回写：发送/回复确认后更新 `campaign_kols.outreach_status`，标记 `sync_status='sync_pending'` 走现有飞书同步；同时更新飞书候选池对应行的"跟进记录/状态"。
...
| 资产 | 位置 | 用法 |
|---|---|---|
| AI 调用逻辑 | `server/routes/finderTasks.js` 约 430–470 行 | 支持 openai 兼容 chat/completions 与 minimax 两种报文；**抽到 `server/services/aiClient.js`**，finderTasks 与邮件模块共用 |
| AI 配置 | `api_settings` 表 `ai.minimax`（已配 key，base_url `https://api.minimaxi.com/v1`），活跃模型选择存 `system.provider_selection` 的 `aiModels.active` | 起草直接复用，不新增配置页 |
| YouTube 快照 | `server/services/youtubeIntakeSnapshot.js` 的 `runYoutubeIntakeSnapshot(customerId)`；结果在 `customers.youtube_*` 字段和 `kol_youtube_snapshot_videos` 表（含 `title/play_count/included_in_aggregate`） | 起草前的证据来源；快照超过 7 天先回抓再起草 |
| 达人-项目状态 | `campaign_kols.outreach_status`、`sync_status` | 发送/回复后更新并置 `sync_pending` |
| 飞书同步 | `server/routes/sync.js`、`server/utils/feishuSubtableMapping.js`、`api_settings` 的 `cloud.feishu_bitable` | 复用；候选池表 `tblhk2nDkERA6jM4`，字段含"跟进记录/状态（已联络/沟通中/已回复）" |
| 策略/brief | `kol_strategies`（`product_context`、`persona_config`、`finder_handoff`） | 起草时注入产品卖点与合作口径 |
| Prompt 模板 | `prompt_templates` 表 + 对应管理页 | 新增一条"外联邮件写作规范"模板，可在设置页编辑 |
...
| POST | `/drafts/generate` | 入参 `{ campaign_id, customer_ids[], kind }`；对每个达人执行第七节流程；返回每个达人的成败 |
| GET | `/drafts` | 过滤：`status, campaign_id, kind, risk_level`；返回计数 `{ pending_review, high_risk, approved }` |
| GET | `/drafts/:id` | 含 evidence 和风险详情 |
| PUT | `/drafts/:id` | 编辑 subject/body_text；仅 `pending_review` 状态可编辑 |
| POST | `/drafts/:id/regenerate` | 入参可选 `{ feedback }`；重新生成，旧版本存入 `email_draft_versions`（见下） |
| POST | `/drafts/:id/approve` | 状态 → `approved` |
| POST | `/drafts/:id/reject` | 入参 `{ reason }` → `rejected` |
| POST | `/drafts/:id/send` | **仅 `approved` 状态可调用**；nodemailer 发送，写 email_records，成功则 draft → `sent`，更新 `campaign_kols.outreach_status='已联络'`、`last_outreach_at`、`sync_status='sync_pending'` |
| GET | `/records` | 发送记录，支持 status 过滤 |
| GET | `/replies` | 回复列表（confirm_status 过滤） |
| POST | `/replies/:id/confirm` / `/ignore` / `/retry-summary` | 落实现有 mock 交互；确认后更新 `campaign_kols.outreach_status`（interested→'已回复'，question→'沟通中'）并置 sync_pending |
...
2. **组装上下文**：
   ...
   - 项目：campaign 名称、`kol_strategies.product_context`（卖点）、合作口径（从 `kind='style_guide'` 的 email_template 读取，含样品+佣金+无固定费+截止日期+授权条款）；
...
| MARKET_MISMATCH | 达人国家与 campaign target_market 不符（如 GB vs US） | high |
...
6. 写入 `email_drafts`（status=`pending_review`）+ `email_draft_versions`。

### 写作规范（style_guide 初始内容，存入 email_templates）

供开发者作为种子数据写入，对应我们已验证的邮件风格：
...
- 新依赖：`imapflow`（需 `npm install`，server 包）。按 `poll_interval_minutes` 轮询 IMAP Unseen 邮件。
...
- 确认/忽略/回写逻辑沿用现有 mock 设计。

## 九、前端改造

### `client/src/pages/Emails.js` 重构为五个 Tab

1. **审批台**（默认）：顶部计数卡（高风险 / 待审阅 / 已批准待发送），可按项目、类型、风险过滤。左侧草稿队列列表（达人、类型 Tag、风险 Tag、生成时间）；右侧上半为可编辑的主题+正文，下半为**证据面板**：引用的视频标题+播放+发布日期、快照日期、均播/中位/粉丝、匹配理由、风险原因列表。操作按钮：保存修改、重新生成（可填反馈）、批准、驳回、批准后发送。
2. **发送记录**：落实现有 mock 列。
...
删除全部 mock 代码和 `USE_MOCK`，改接真实 API。
...
3. 未批准的草稿调 `/send` 返回 409；批准后发送成功，email_records 有记录，`campaign_kols.outreach_status='已联络'`，飞书候选池对应行"跟进记录"在下次同步后更新。
...
1. migration + models + aiClient 抽取（finderTasks 改引用，回归测试）。
