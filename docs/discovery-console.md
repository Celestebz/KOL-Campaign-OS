...
1. 选择已发布的项目产品搜索方案、一个平台、1–50 人目标，填写具体要求。
...
首次仍需发布产品策略；本版不会自动生成或发布策略。网页不会启动
...
新增迁移：`20260918000001-create-discovery-requests.js`，只新增
`discovery_requests` 表及索引；不搬迁历史达人数据。生产需先备份数据库，
