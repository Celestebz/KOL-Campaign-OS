...
const { productContextSql, buildDiscoveryContext } = require('./discoveryContext');
...
function createDiscoveryService({ model, sequelize, query, createFinderTask }) {
  const publicData = (row) => {
    ...
    try { data.context = JSON.parse(data.context_json || 'null'); } catch { data.context = null; }
    delete data.context_json;
  ...
  const binding = async (campaignId, productId, transaction) => {
    const [row] = await query(productContextSql, [positiveId(campaignId), positiveId(productId)], transaction);
    if (!row) throw error(409, '请选择有效项目及该项目中的启用产品');
    return row;
  };
  async function describe(row) {
    const data = publicData(row);
    const [context] = await query(`SELECT c.name AS campaign_name, p.name AS product_name
      FROM campaigns c LEFT JOIN campaign_products cp ON cp.id = ? AND cp.campaign_id = c.id
      LEFT JOIN products p ON p.id = cp.product_id WHERE c.id = ?`, [row.campaign_product_id, row.campaign_id]);
  ...
  async function create(userId, body) {
    const input = { owner_user_id: owner(userId), request_key: key(body.request_key),
      campaign_id: positiveId(body.campaign_id), campaign_product_id: positiveId(body.campaign_product_id), target_platform: body.target_platform,
    ...
    if (existing) {
      if (['campaign_id', 'campaign_product_id', 'target_platform', 'target_count', 'requirements'].some((field) => existing[field] !== input[field])) throw error(409, '此提交标识已用于其他需求，请刷新后重试');
      return describe(existing);
    }
    const context = await binding(input.campaign_id, input.campaign_product_id);
    try {
      return await describe(await model.create({ ...input, strategy_id: null, context_json: JSON.stringify(context), status: 'queued', stage: 'waiting' }));
    } catch (e) {
  ...
  async function claim(userId, id, body) {
    ...
    const row = await sequelize.transaction(async (transaction) => {
      ...
      const context = await binding(request.campaign_id, request.campaign_product_id, transaction);
      if (!request.finder_task_id) {
        const task = await createFinderTask({ strategyId: request.strategy_id, targetPlatform: request.target_platform,
          discoveryContext: request.strategy_id ? null : buildDiscoveryContext(JSON.parse(request.context_json || 'null') || context, request.requirements),
  ...
  return { create, claim, active, progress, control,
    catalog: async () => query(`SELECT c.id AS campaign_id, c.name AS campaign_name,
      cp.id AS campaign_product_id, p.name AS product_name, p.brand AS product_brand
      FROM campaigns c JOIN campaign_products cp ON cp.campaign_id = c.id
      JOIN products p ON p.id = cp.product_id
      WHERE c.status = 'active' AND cp.status = 'active' AND p.status = 'active'
      ORDER BY c.id, cp.id`),
